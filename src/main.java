...
public class main {
    ...
    private static void start() {
        ...
        String result = getString(originalDescriptor);
        System.out.println(result);
    }

    private static String getString(String originalDescriptor) {
        String[] lines = originalDescriptor.split("\n");  // 문자열을 줄 단위로 분리하여 배열에 저장
        StringBuilder sb = new StringBuilder();  // 결과를 저장할 StringBuilder 객체 생성
        for (int i = 2; i < lines.length; i += 3) {  // 인덱스 2부터 3씩 증가하며 3번째 줄씩 가져옴
            sb.append(lines[i]).append("\n");  // StringBuilder 객체에 줄 단위로 추가
        }
        String result = sb.toString();  // StringBuilder 객체를 문자열로 변환하여 저장
        return result;
    }
